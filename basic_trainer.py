    ...
    def train(self, dataset_handler, verbose=False):
        ...
        for epoch in tqdm(range(1, self.epochs + 1)):
            ...
            for batch in dataset_handler.train_dataloader:
                # Doc BoW to target device (FP32)
                x = batch['data'].to(self.device, dtype=torch.float32, non_blocking=True)

                # Build (B*S, V) sparse COO directly on device (FP32)
                ...
                shape = batch['sub_shape']
                B, S = batch['B'], batch['S']

                idx = torch.stack([rows, cols], dim=0)  # (2, nnz)
                X_bs = torch.sparse_coo_tensor(idx, vals, shape, device=self.device).coalesce()
                x_sub = (X_bs, B, S)  # tuple path in your model

                # Forward + backward (pure FP32)
